"""
A8 - Feature-Flagged Origin Label Handling
Polling every 15 min.
Runs first in each polling cycle (see run_polling_suite.sh).

When shared Quality Gate mode is disabled, preserves legacy behavior and silently adds `us-taxops-ticket` only to PF Ops-Customer-Task tickets
...
When shared Quality Gate mode is enabled, this script is report-only and does not add, remove, or guess origin labels.
"""
import sys, os
...
def _run_shared_report_only():
    print("[A8] shared Quality Gate mode enabled; origin labels must be explicit; no auto-stamping performed")
...
def run():
    if shared_quality_gate_enabled():
        return _run_shared_report_only()
    return _run_legacy()
